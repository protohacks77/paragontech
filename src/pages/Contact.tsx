  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-4 grid lg:grid-cols-2 gap-12">
        ...
        >
          <GlassCard className="p-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-orbitron text-2xl font-bold text-black">
                Get A Quote
              </h2>
              <motion.button
            ...
            {showQuoteCalculator && (
              <motion.div
                ...
                className="mb-6 p-4 bg-black/5 rounded-lg border border-black/10"
              >
                <h3 className="font-semibold text-black mb-4">Project Estimator</h3>
                
                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  <div>
                    <label className="block text-black/70 mb-2">Project Type</label>
                    <select
                      value={quoteData.projectType}
                      onChange={(e) => setQuoteData(prev => ({ ...prev, projectType: e.target.value }))}
                      className="w-full p-3 bg-black/10 border border-black/20 rounded-lg text-black focus:outline-none focus:border-cyber-purple"
                    >
                  ...
                  <div>
                    <label className="block text-black/70 mb-2">Complexity</label>
                    <select
                ...
                <div className="text-center p-4 bg-gradient-to-r from-cyber-purple/20 to-cyber-blue/20 rounded-lg">
                  <p className="text-black/70">Estimated Cost:</p>
                  <p className="text-2xl font-bold text-cyber-purple">${calculateEstimate().toLocaleString()}</p>
                  <p className="text-black/60 text-sm">*Final pricing may vary based on specific requirements</p>
                </div>
              </motion.div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-black/70 mb-2">Name *</label>
                  <input
                    type="text"
                    name="name"
                    value={formData.name}
                    onChange={handleInputChange}
                    className="w-full p-3 bg-black/10 border border-black/20 rounded-lg text-black placeholder-black/50 focus:outline-none focus:border-cyber-purple transition-colors"
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-black/70 mb-2">Service Needed</label>
                  <select
                    name="service"
                ...
                <div>
                  <label className="block text-black/70 mb-2">Budget Range</label>
                  <select
                    name="budget"
              ...
              <div>
                <label className="block text-black/70 mb-2">Project Details *</label>
                <textarea
                  name="message"
                  value={formData.message}
                  onChange={handleInputChange}
                  rows={5}
                  className="w-full p-3 bg-black/10 border border-black/20 rounded-lg text-black placeholder-black/50 focus:outline-none focus:border-cyber-purple transition-colors resize-none"
        ...
        >
          <GlassCard className="p-8">
            <h2 className="font-orbitron text-2xl font-bold text-black mb-6">
              Get In Touch
            </h2>
            ...
            <div className="space-y-4">
              {contactInfo.map((info, index) => (
                <motion.a
                  ...
                  whileHover={{ scale: 1.02 }}
                  className="flex items-center space-x-4 p-4 bg-black/5 rounded-lg hover:bg-black/10 transition-colors group"
                >
                  <div className="text-cyber-purple group-hover:text-cyber-blue transition-colors">
                    {info.icon}
                  </div>
                  <div>
                    <p className="text-black/60 text-sm">{info.title}</p>
          ...
          <GlassCard className="p-8">
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-black/70">Email responses within 2 hours</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
                <span className="text-black/70">Project quotes within 24 hours</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                <span className="text-black/70">Initial consultation scheduled within 48 hours</span>
              </div>
            </div>
          </GlassCard>
        </motion.div>
      ...
      <section className="py-20 px-4">
        <div className="max-w-6xl mx-auto">
          <GlassCard className="p-8">
            <h2 className="font-orbitron text-2xl font-bold text-black mb-6 text-center">
              Our Location
            </h2>
            <div className="bg-light-800 rounded-lg h-64 flex items-center justify-center">
